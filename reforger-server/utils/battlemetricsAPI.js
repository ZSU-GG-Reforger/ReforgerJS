...
class BattleMetricsAPI {
  ...
  async createBan({ identifiers, reason, note, expires, permanent = false }) {
    try {
      if (!this.initialized)
        throw new Error("BattleMetrics API not initialized");
      ...
      logger.info(
        `BattleMetrics ban created successfully: ${response.data.data.id}`
      );
      return response.data;
    } catch (error) {
      logger.error(`Error creating BattleMetrics ban: ${error.message}`);
      if (error.response) {
        logger.error(`API response: ${JSON.stringify(error.response.data)}`);
      }
      throw error;
...
async createPlayerNote(playerIdentifier, attributes = {}, isReforgerUUID = false) {
  try {
    ...
    logger.verbose(`Testing API permissions by checking existing notes for player ${playerId}...`);
    try {
      const notesEndpoint = `${this.baseUrl}/players/${playerId}/relationships/notes`;
      const existingNotes = await axios.get(notesEndpoint, { headers: this.headers });
      
      if (existingNotes.data && existingNotes.data.data) {
        logger.verbose(`Found ${existingNotes.data.data.length} existing notes for player ${playerId}`);
      } else {
        logger.verbose(`No existing notes found or unexpected response format`);
      }
      
      logger.verbose(`Successfully retrieved notes - we have read permissions`);
    } catch (readError) {
      logger.error(`Cannot read existing notes: ${readError.message}`);
      if (readError.response) {
        logger.error(`API response on read test: ${readError.response.status} - ${JSON.stringify(readError.response.data)}`);
      }
    }
    
    const endpoint = `${this.baseUrl}/players/${playerId}/relationships/notes`;
    logger.verbose(`Using endpoint for creating note: ${endpoint}`);
...
async deletePlayerFlag(playerIdentifier, flagId, isReforgerUUID = false) {
  ...
  /**
   * Create a ban for a player with only a Reforger UUID
   ...
   * @param {boolean} options.permanent - Whether the ban is permanent
   * @returns {Promise<Object|null>} Ban data if successful, null otherwise
   */
  async createBanByReforgerUUID(
    reforgerUUID,
    { reason, note, expires, permanent = false }
  ) {
    try {
      if (!this.initialized)
        throw new Error("BattleMetrics API not initialized");
